...
export class KaseyaProvider implements RmmProvider {
  ...
  async validateCredentials(credentials: any) {
    ...
    try { return (await fetch(`${this.baseUrl(credentials)}/api/v1.0/assetmgmt/agents?$top=1`, { headers: this.headers(credentials), signal: AbortSignal.timeout(10000) })).ok; } catch { return false; }
  }
